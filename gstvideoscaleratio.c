...
static void
gst_videoscaleratio_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
static gboolean
gst_videoscaleratio_sink_event (GstPad *pad, GstObject *parent, GstEvent *event)
{
...
gst_videoscaleratio_init (GstVideoScaleRatio *filter)
{
    ...
    gst_bin_add_many (GST_BIN (filter), filter->videoscale, filter->capsfilter, NULL);
